                                         ...
                                         ULONG alloc_type, ULONG protect, USHORT machine )
{
    ...
    if (NT_SUCCESS(res))
    {
         ...
         * loader sections unpacked via NtMapViewOfSection, which alias a single
         * section as RW and RX instead of ever requesting RWX, and so never
         * pass MEM_EXTENDED_PARAMETER_EC_CODE).
                                         ...
                                         ULONG_PTR align, ULONG attributes )
{
     ...
     * Test the Win32 protection rather than vprot: vprot is only assigned on
     * the MEM_RESERVE path, and is uninitialised when committing. */
    if (!status && arm64ec_view && !(attributes & MEM_EXTENDED_PARAMETER_EC_CODE) &&
        (protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE |
                    PAGE_EXECUTE_WRITECOPY)) && is_view_valloc( view ))
    {
        commit_arm64ec_map( view );
        set_arm64ec_range( base, size );
    }
